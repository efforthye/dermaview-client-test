...
export const ViewerCore = (props: ViewerProps) => {
  const {
    visible = false,
    onClose = noop,
    images = [],
  ...
  return (
    ...
    >
      ...
      {props.noClose || (
        <div
          className="absolute w-10 h-10 z-10000 bg-white bg-opacity-20 rounded-lg cursor-pointer flex justify-center items-center text-white top-2 right-2 z-[100] hover:bg-opacity-50"
          onClick={() => {
            onClose();
          }}
        >
      ...
      {props.noFooter || (
        <div
          className="fixed right-0 bottom-0 left-0 overflow-hidden text-center z-[100]"
    ...
    <p className={`${props.prefixCls}-attribute`}>
      ...
      {props.showTotal && (
        <span className={`${props.prefixCls}-showTotal`}>
          {`${props.activeIndex ?? 0 + 1} ${props.totalName} ${props.count}`}
        </span>
      )}
    </p>
  ) : null;
